...
def ffnn(inputs, num_hidden_layers, hidden_size, output_size, dropout, output_weights_initializer=None):
    ...
    output_weights = tf.compat.v1.get_variable("output_weights", [shape(current_inputs, 1), output_size], initializer=output_weights_initializer)
...
class CustomLSTMCell(tf.compat.v1.nn.rnn_cell.RNNCell):
    def __init__(self, num_units, batch_size, dropout):
        ...
        self._dropout_mask = tf.nn.dropout(tf.ones([batch_size, self.output_size]), 1 - (dropout))
